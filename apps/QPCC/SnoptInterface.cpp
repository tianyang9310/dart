...
SnoptInterface::SnoptInterface(int has_obj, VVD J, VVB JMap, std::vector<double> *constraints, double *objective, std::vector<double> *gradient, SnoptInterface::updateFunc update_f, void *update_d) {
    mHasObjective = has_obj;
...
#endif
extern "C" {
    void SNINIT(long int *iprint, long int *isum, char *cw, long int *lencw,
                long int *iw, long int *leniw, double *rw, long int *lenrw);
    void SNOPT(char *start, long int *m, long int *n, long int *ne,
               ...
               long int start_len);
    void SNSPEC(long int *ispecs, long int *inform, char *cw, long int *lencw,
                long int *iw, long int *leniw, double *rw, long int *lenrw);
...
SnoptInterface::Return SnoptInterface::solve(double *x, double *lo_bounds,
                                             double *hi_bounds, long int unit) {
    ...
    long int constr_count = (1 > mNumConstr)? 1 : mNumConstr;
    long int nm = mNumCoef + constr_count;
    ...
    static long int spec_loaded = 0;
    // Because SNSPECT can't be loaded twice (some run-time Fortran IO error would be thrown otherwise), we use static variables to keep the information in the spec file for running snopt problems multiple times.
    
    long int iprint = mOutput;
    long int ispec = unit; //if spcname is not specified, snopt will load in fort.4 as default
    ...
    long int nName = 1;
    long int nnCon = mNumNonlinConstr;
    long int nnObj = mNumNonlinObjCoef;
    long int nnJac = (nnCon == 0) ? 0 : mNumNonlinJacCoef;
    long int iObj = 0;
    double ObjAdd = 0;
