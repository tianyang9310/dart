...
void myGlutKeyboard(unsigned char key, int x, int y) {
    ...
    case '2': {         //part 2 of project 1 - tinker toy
        ...
        break; }
    case 'f': {
        cout<<"Part 2: Change to Forward Euler Method"<<endl;
        mySimulator->Integrator = 0;
        break;}
    case 'm': {
        cout<<"Part 2: Change to Mid Point Method"<<endl;
        mySimulator->Integrator = 1;
        break;}
    case 'k': {
        cout<<"Part 2: Change to RK4 Method"<<endl;
        mySimulator->Integrator = 2;
        break;}
    case 'i': {
        cout<<"Part 2: Change to Implicit Euler Method"<<endl;
        mySimulator->Integrator = 3;
        break;}
    default:
            break;
    }
...
int FindParticle(shared_ptr<Simulator> mySimulator, int x_old, int y_old)
{
    double ratio = 735.0;
    int res = -1;
    double dis = 100000;
    double x_position = (x_old - 400)/ratio;
    double y_position =-(y_old - 300)/ratio;
    // cout<<x_position<<endl;
    // cout<<y_position<<endl;
    for (unsigned int i=0; i<mySimulator->getNumParticles();i++)
    {
        double cur_dis = sqrt(pow(x_position-mySimulator->mParticles[i].mPosition[0],2)+pow(y_position-mySimulator->mParticles[i].mPosition[1],2));
        // cout<<cur_dis<<endl;
        if (cur_dis<dis)
        {
            dis=cur_dis;
...
void myGlutMouse(int button, int state, int x, int y) {
    ...
    else if (button == GLUT_LEFT_BUTTON && state == GLUT_UP)
    {
        ...
        y_new=y;
        // apply force
        int flag = FindParticle(mySimulator,x_old,y_old);
        // cout<<"Nearest particle is "<< flag <<endl;
        double magnitude = 0.05;
        Eigen::Vector3d ext_force;
        ext_force<< magnitude*(x_new-x_old),-magnitude*(y_new-y_old), 0;
        cout<<"Apply to Particle "<<flag<<": "<<ext_force.transpose()<<endl;
        mySimulator->flag = flag;
        mySimulator->ExtForce = ext_force;
    }
